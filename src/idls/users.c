...
static ret_t user_detail(user_t* user, const char* args) {
  return RET_OBJECT_CHANGED;
}

static bool_t user_can_exec_sort(user_t* user, const char* args) {
  return TRUE;
}

static ret_t user_sort(user_t* user, const char* args) {
  return RET_OBJECT_CHANGED;
}
...
static ret_t users_view_model_get_prop(object_t* obj, const char* name, value_t* v) {
  ...
  if (tk_str_eq("name", name)) {
    ...
    value_set_str(v, user->filter.str);
...
static bool_t users_view_model_can_exec(object_t* obj, const char* name, const char* args) {
  ...
  if (tk_str_ieq(name, "remove")) {
    ...
    return user_can_exec_sort(user, args);
  } else {
    return FALSE;
  }
}
...
static ret_t users_view_model_exec(object_t* obj, const char* name, const char* args) {
  ...
  if (tk_str_ieq(name, "remove")) {
    ...
    return user_detail(user, args);
  } else if (tk_str_eq("sort", name)) {
    return user_sort(user, args);
  } else {
    log_debug(
        "not found %s\
",
...
view_model_t* users_view_model_create(navigator_request_t* req) {
  ...
  darray_init(&(user_vm->users), 100, (tk_destroy_t)user_destroy, (tk_compare_t)user_cmp);
